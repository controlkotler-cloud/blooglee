...
const BlogIndex = () => {
  ...
  const selectedAudience = audienceParam === "empresas" || audienceParam === "agencias" ? audienceParam : "todos";
  ...
  const {
    ...
    isLoading,
    error,
  } = useBlogPosts(selectedAudience === "todos" ? undefined : selectedAudience, selectedCategory);
  ...
  return (
    <PublicLayout>
      <SEOHead
        title="Blog de SEO, Marketing de Contenidos y Automatización WordPress"
        description="Guías prácticas de SEO, marketing de contenidos y automatización con IA para hacer crecer tu blog WordPress. Casos reales, tutoriales y estrategias para empresas y agencias."
        canonicalUrl="/blog"
        keywords="blog SEO, marketing contenidos, automatización blog, tutoriales WordPress, estrategia digital, SEO empresas, SEO agencias"
      />
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-12 lg:py-16">
        ...
        <div>
                ...
                <div className="flex items-center gap-0.5 sm:gap-1">
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
                    ...
                    if (!showOnMobile && totalPages > 5) {
                      ...
                      if (isEllipsisBefore) {
                        return (
                          <span key={page} className="sm:hidden text-muted-foreground px-1">
                            …
                          </span>
                        );
                      }
          ...
          <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-white/50 shadow-xl p-6 sm:p-8 mt-8 sm:mt-12 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <Button
              asChild
              className="w-full sm:w-auto shrink-0 bg-gradient-to-r from-violet-500 to-fuchsia-500 hover:opacity-90"
            >
              <Link to="/waitlist">
                Empezar gratis

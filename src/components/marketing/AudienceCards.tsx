...
export function AudienceCards({ selected, onSelect, counts }: AudienceCardsProps) {
  const cards = [
    {
      id: 'empresas',
      title: 'Para empresas',
  ...
  // If "todos" is selected, show the cards prominently
  if (selected === 'todos') {
    return (
      <div className="mb-8">
        ...
        <div className="relative my-8">
          ...
          <div className="relative flex justify-center">
            <span className="bg-gradient-to-r from-violet-50 via-white to-violet-50 px-4 text-sm text-foreground/50">
  ...
  // When a specific audience is selected, show compact tabs
  return (
    <div className="flex gap-2 mb-6">
      <button
        onClick={() => onSelect('todos')}
        className="px-4 py-2 rounded-xl text-sm font-medium bg-white/70 text-foreground/70 hover:bg-white hover:text-foreground border border-white/50 transition-all"
      >
        ← Todos
      </button>
      {cards.map((card) => {
        ...
        return (
          ...
          >
            ...
            {card.count > 0 && (
              <span className={`px-1.5 py-0.5 rounded-full text-xs ${isSelected ? 'bg-white/20' : 'bg-foreground/10'}`}>
                {card.count}
              </span>
            )}
          </button>

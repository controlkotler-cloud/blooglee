...
export function ArticleCard({ article, onView, onPublish, onDelete }: ArticleCardProps) {
  ...
  return (
    <Card className="group hover:shadow-md transition-shadow overflow-hidden">
      ...
      <div className="relative aspect-video bg-muted">
        {article.image_url ? (
          <img src={article.image_url} alt={title} loading="lazy" className="w-full h-full object-cover" />
        ) : (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1">
            {article.wp_post_url && (
              <Badge
                variant="secondary"
                className="text-xs px-1.5 py-0 text-emerald-600 bg-emerald-50 border-emerald-200"
              >

...
export function ProgressBar({ currentStep, totalSteps = 5 }: ProgressBarProps) {
  return (
    <div
      className="flex items-center justify-center w-full pt-3 sm:pt-4 pb-5 sm:pb-8 mb-2 sm:mb-4"
      role="progressbar"
      aria-valuenow={currentStep}
      aria-valuemin={1}
      aria-valuemax={totalSteps}
      aria-label={`Paso ${currentStep} de ${totalSteps}: ${STEP_LABELS[currentStep - 1] || ""}`}
    >
              ...
              <div className="flex flex-col items-center">
                <div
                  className={cn(
                    ...
                    isActive &&
                      "bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white shadow-md animate-[onboarding-pulse_2s_ease-in-out_infinite]",
                    !isCompleted && !isActive && "bg-muted text-muted-foreground",
                  )}
                  aria-label={`Paso ${step}: ${STEP_LABELS[i]}${isCompleted ? " — completado" : isActive ? " — actual" : ""}`}
                >
              ...
              {step < totalSteps && (
                <div
                  className={cn(
                    ...
                    "sm:mt-[-16px]",
                    currentStep > step ? "bg-gradient-to-r from-violet-500 to-fuchsia-500" : "bg-border",
                  )}
                />
              )}
            </div>

...
export function BusinessStep({
  ...
  const [businessType, setBusinessType] = useState(initialData?.business_type ?? getDefaultBusinessType(initialData?.sector));
  ...
  const [editorialFocusSuggestion, setEditorialFocusSuggestion] = useState(initialData?.editorial_focus_suggestion ?? '');
  ...
  useEffect(() => {
    ...
    setBusinessType((prev) => prev || extractedProfile.business_type || getDefaultBusinessType(extractedProfile.sector));
  ...
  const canAnalyze = websiteUrl.trim().length > 0 && urlAnalysis.status !== 'invalid' && extractionStatus !== 'extracting';
  const canProceed =
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const normalizedUrl = primaryBlogUrl.toLowerCase().replace(/\/+$/, '').replace(/^https?:\/\//, '').replace(/^www\./, '');
    ...
    <div className="space-y-5">
      <div className="text-center space-y-2 mb-4 sm:mb-6">
        ...
        <p className="text-muted-foreground text-sm max-w-md mx-auto">
          Analizamos tu web para proponerte una base de configuración. Después solo revisas, ajustas y generas tu primer post.
        </p>
      </div>
      ...
      <div className="space-y-3 rounded-xl border bg-card p-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button type="button" onClick={handleAnalyzeWebsite} disabled={!canAnalyze} className="gap-2 self-start">
            {extractionStatus === 'extracting' ? <Loader2 className="w-4 h-4 animate-spin" /> : <WandSparkles className="w-4 h-4" />}
      ...
      {(extractedProfile || initialData?.extracted_from_website || isApplyingDetection) && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
          Hemos detectado esta información a partir de tu web. Revísala y modifícala si algo no encaja del todo con tu negocio.
        </div>
      )}
      ...
      {detectedInfoExists && (
        <div className="rounded-xl border bg-card p-4 space-y-3">
          ...
          <div className="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
            {detectedBlogUrl && <p><span className="font-medium text-foreground">Blog:</span> {detectedBlogUrl}</p>}
            {detectedSocialUrl && <p><span className="font-medium text-foreground">Red social:</span> {detectedSocialUrl}</p>}
            {detectedLanguages.length > 0 && <p><span className="font-medium text-foreground">Idiomas:</span> {detectedLanguages.join(', ')}</p>}
            {detectedColors.length > 0 && <p><span className="font-medium text-foreground">Colores:</span> {detectedColors.join(', ')}</p>}
            {toneSuggestion && <p><span className="font-medium text-foreground">Tono sugerido:</span> {toneSuggestion}</p>}
            {audienceSuggestion && <p><span className="font-medium text-foreground">Audiencia sugerida:</span> {audienceSuggestion}</p>}
            {contentGoalSuggestion && <p><span className="font-medium text-foreground">Objetivo sugerido:</span> {contentGoalSuggestion}</p>}
            {editorialFocusSuggestion && <p><span className="font-medium text-foreground">Enfoque sugerido:</span> {editorialFocusSuggestion}</p>}
            {keywordsSuggestion && <p><span className="font-medium text-foreground">Keywords detectadas:</span> {keywordsSuggestion}</p>}
          </div>
        </div>
      )}

...
async function shouldGenerateForSite(
  ...
  if (normalizedFrequency === "daily" || normalizedFrequency === "daily_weekdays") {
    if (normalizedFrequency === "daily_weekdays") {
  ...
  if (normalizedFrequency === "biweekly") {
    ...
    if (weekOfMonth !== 1 && weekOfMonth !== 3) {
      return { shouldGenerate: false, reason: "Semana fuera de quincenal (solo 1 y 3)" };
    }
    ...
    return {
      shouldGenerate: !data || data.length === 0,
      reason: data?.length ? "Ya tiene artículo esta quincena" : undefined,
    };
  }
...
async function checkPlanLimits(
  ...
  const { data: profile } = await supabase.from("profiles").select("posts_limit, email").eq("user_id", userId).single();
...
async function getUsedTopics(supabase: SupabaseClient, siteId: string): Promise<string[]> {
  const { data } = await supabase
    .from("articles")
    .select("topic")
    .eq("site_id", siteId)
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(
      `Processing for ${DAY_NAMES[now.getDay()]} ${now.getDate()} ${MONTH_NAMES[currentMonth - 1]} ${currentYear}`,
    );
    ...
    if (sitesError) {
      ...
      for (const site of sites || []) {
        ...
        const check = await shouldGenerateForSite(
          supabase,
          site.id,
          normalizeFrequency(site.publish_frequency || "monthly"),
          currentMonth,
          currentYear,
          now,
        );
    ...
    console.log(
      `Generated: ${successCount}, Skipped: ${skippedCount}, Failed: ${failCount}, Limit Exceeded: ${limitExceededCount}, WP: ${wpPublished}, Blog: ${blogGeneratedCount}`,
    );
